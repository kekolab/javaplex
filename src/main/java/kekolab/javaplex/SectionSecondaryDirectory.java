...
public class SectionSecondaryDirectory extends Directory implements PlexSectionSecondaryDirectory {
    private UriProvider fastKey;
    private String type;

    public SectionSecondaryDirectory() {
    ...
    <T extends PlexMetadata> List<T> apply(PlexSection section, Integer type) {
        try {
            ...
            URI uri = new URIBuilder(section.key()).appendPath("all")
                    ...
                    .build();
            return new MetadataContainer<T, Directory>(uri, getServer()).getMetadata();
